...
function card(product){
  return `<article class="product-card category-product-card"><div class="product-art"><img src="${product.image}" alt="${esc(product.name)} product image" loading="lazy"></div><div class="product-card-content"><p class="product-category">${categoryLabels[product.category]}</p><h3>${esc(product.name)}</h3><p>${esc(product.tagline)}</p><small class="feature-line">${esc(product.features)}</small><div class="card-actions"><a class="card-link" href="product.html?id=${product.id}">View product ↗</a><span class="price">From $${product.price.toLocaleString()}</span></div></div></article>`;
}
